...
// Finds the stop in a route whose (base) name overlaps with the given free-text
// location string (what the user typed or picked) — checked both ways, since
// the text might be a long resolved address containing a short stop name
// ("...ဆူးလေဘုရားလမ်း..." contains "ဆူးလေ"), or a short landmark the user typed
// directly that a longer stop name contains ("တာမွေ" is contained in
// "တာမွေဈေး"). Prefers the longest overlap, since short names ("ဈေး", "ဂိတ်")
// match too many places to be a reliable signal on their own.
function findStopMatch(stops, text) {
  const normText = normalizeForMatch(text);
  if (!normText) return -1;
  let bestIdx = -1;
  let bestLen = 1; // require at least 2 characters to count as a real match
  stops.forEach((stop, i) => {
    const s = normalizeForMatch(baseStopName(stop));
    if (s.length <= bestLen) return;
    const overlaps = normText.includes(s) || s.includes(normText);
    if (overlaps) {
      bestLen = s.length;
      bestIdx = i;
    }
...
function findDirectYbsRoutes(startText, destText, routesData) {
  ...
  for (const [key, route] of Object.entries(routesData)) {
    for (const dirKey of ["forward", "backward"]) {
      ...
      const startIdx = findStopMatch(dir.stops, startText);
      const destIdx = findStopMatch(dir.stops, destText);
      if (startIdx === -1 || destIdx === -1 || destIdx <= startIdx) continue;

      results.push({
        route: key,
        board: dir.stops[startIdx],
        alight: dir.stops[destIdx],
        hops: destIdx - startIdx,
      });
    }
  }

  // Fewer stops between boarding and alighting = a more direct, easier-to-verify match.
    ...
    .map(
      ...
      <div class="ride-option">
        ...
        <div class="ride-option-body">
          ...
          <div class="ride-option-sub">${board} တက် → ${alight} ဆင်း</div>
        </div>
